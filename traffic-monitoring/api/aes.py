...
def hex_to_bytes(hex_str):
    return np.array([int(hex_str[i:i + 2], 16) for i in range(0, len(hex_str), 2)], dtype=np.uint8).reshape(4, 4).T
...
def key_expansion(key):
    ...
    for i in range(1, 11):
        temp = np.roll(round_keys[-1][:, 3], -1)
        temp = np.array([s_box[b >> 4][b & 0x0F] for b in temp])
        temp[0] ^= rcon[i - 1]
        new_key = round_keys[-1][:, 0] ^ temp
...
def text_to_hex(text):
    ...
    return ''.join([f'{ord(c):02x}' for c in padded_text])

def hex_to_bytes(hex_str):
    # Ensure the hex string is of the correct length (must be even)
    if len(hex_str) % 2 != 0:
        raise ValueError("Hex string length must be even.")
    return np.array([int(hex_str[i:i + 2], 16) for i in range(0, len(hex_str), 2)], dtype=np.uint8).reshape(-1, 4).T

def aes_encrypt(plaintext_text, key_hex):
